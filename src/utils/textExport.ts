...
export function generateTextItinerary(plan: Plan, lang: LangType = 'zh'): string {
    ...
    dayKeys.forEach((dayKey, idx) => {
        ...
        if (dayTotal > 0) {
            lines.push(`  💰 ${isZh ? '預估花費' : 'Est. cost'}: ¥${dayTotal.toLocaleString()}`);
            lines.push('');
        }
    });
    ...
    lines.push(`${'━'.repeat(30)}`);
    if (grandTotal > 0) {
        lines.push(`💰 ${isZh ? '全程預估總花費' : 'Total estimated cost'}: ¥${grandTotal.toLocaleString()}`);
    }
...
export function downloadPdf(plan: Plan, lang: LangType = 'zh'): void {
    ...
    dayKeys.forEach((dayKey, idx) => {
        ...
        const dayTotalHtml = dayTotal > 0 ? `<div style="text-align:right;color:#0d9488;font-weight:700;font-size:13px;padding-top:8px;border-top:1px solid #e5e7eb">💰 ${isZh ? '預估花費' : 'Est.'}: ¥${dayTotal.toLocaleString()}</div>` : '';
    ...
    const grandTotalHtml = grandTotal > 0 ? `<div style="text-align:center;font-size:16px;font-weight:800;color:#0d9488;padding:16px;background:#f0fdfa;border-radius:12px;margin-top:24px">💰 ${isZh ? '全程預估總花費' : 'Total'}: ¥${grandTotal.toLocaleString()}</div>` : '';
