...
const ItemDetailModal: React.FC<ItemDetailModalProps> = ({ isOpen, onClose, item, t, lang = 'zh', onUpdateScheduleItem, onUpdateCustomAsset }) => {
    ...
    const author = item.authorId ? SAMPLE_CREATORS.find(c => c.id === item.authorId) : null;
    const displayAuthorName = lang === 'en' && author?.nameEn ? author.nameEn : (author?.name || (lang === 'zh' ? 'Canvas 官方' : 'Canvas Official'));
    ...
    const displayDescription = lang === 'en' && (item as any).descriptionEn ? (item as any).descriptionEn : (item.description || '');
    const displayDuration = lang === 'en' && item.duration ? item.duration.replace('小時', 'h').replace('分鐘', 'm') : item.duration;
    const displayImage = item.coverImage || (item as any).image;
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar scroll-smooth pb-32">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-sm font-black text-slate-900 leading-tight">
                                        {author?.name || item.author || (lang === 'zh' ? 'Canvas 官方指南' : 'Travel Canvas Official')}
                                    </span>
                                </div>
                            </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3 px-8 mb-10">
                        <div className="glass-card p-4 rounded-3xl border-slate-100 shadow-sm flex flex-col items-center">
                            ...
                            <span className="text-lg font-black text-slate-900">¥{item.price?.toLocaleString() || '0'}</span>
                        </div>
                        <div className="glass-card p-4 rounded-3xl border-slate-100 shadow-sm flex flex-col items-center">
                        ...
                        <div className="hidden md:flex glass-card p-4 rounded-3xl border-slate-100 shadow-sm flex flex-col items-center">
                            ...
                            <span className="text-lg font-black text-slate-900">{displayDuration || '1h'}</span>
                        </div>
                    </div>
                                ...
                                <div className="p-8 pt-10 rounded-[3rem] relative overflow-hidden bg-white text-slate-900 z-10 border border-slate-100 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.08)] transition-all hover:shadow-[0_25px_60px_-12px_rgba(0,0,0,0.12)]">
                                    ...
                                    <div className="relative z-10 flex flex-col gap-6">
                                        ...
                                        <div className="space-y-6">
                                            {(item.insiderTip?.teaser || item.insiderTip?.teaserEn) && (
                                            ...
                                            <div className="space-y-4">
                                                <div className="relative">
                                                    ...
                                                    <p className="text-sm text-slate-600 leading-relaxed font-bold italic relative z-10 pl-6 mb-6">
                                                        {lang === 'zh' ? item.insiderTip?.full?.story : (item.insiderTip?.full?.storyEn || item.insiderTip?.full?.story)}
                                                    </p>
                                                    ...
                                                    <div className="flex items-center gap-3 p-4 bg-slate-50/80 rounded-2xl border border-slate-100/50 hover:bg-slate-50 transition-colors">
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="text-xs font-black text-slate-800 leading-snug">
                                                                {lang === 'zh' ? item.insiderTip?.full?.exactLocation : (item.insiderTip?.full?.exactLocationEn || item.insiderTip?.full?.exactLocation)}
                                                            </div>
                                                        </div>
                                                    </div>
                                                )}
                                                {item.insiderTip.full.mustTry && (
                                                    <div className="flex items-center gap-3 p-4 bg-slate-50/80 rounded-2xl border border-slate-100/50 hover:bg-slate-50 transition-colors">
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="text-xs font-black text-slate-800 leading-snug">
                                                                {lang === 'zh' ? item.insiderTip?.full?.mustTry : (item.insiderTip?.full?.mustTryEn || item.insiderTip?.full?.mustTry)}
                                                            </div>
                                                        </div>
                                                    </div>
                                                )}
                                                {item.insiderTip.full.bestTime && (
                                                    <div className="flex items-center gap-3 p-4 bg-slate-50/80 rounded-2xl border border-slate-100/50 hover:bg-slate-50 transition-colors">
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="text-xs font-black text-slate-800 leading-snug">
                                                                {lang === 'zh' ? item.insiderTip?.full?.bestTime : (item.insiderTip?.full?.bestTimeEn || item.insiderTip?.full?.bestTime)}
                                                            </div>
                                                        </div>
                                                    </div>
                                                )}
                                                {item.insiderTip.full.avoid && (
                                                    <div className="flex items-center gap-3 p-4 bg-slate-50/80 rounded-2xl border border-slate-100/50 hover:bg-slate-50 transition-colors">
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="text-xs font-black text-slate-800 leading-snug">
                                                                {lang === 'zh' ? item.insiderTip?.full?.avoid : (item.insiderTip?.full?.avoidEn || item.insiderTip?.full?.avoid)}
                                                            </div>
                                                        </div>
                                                    </div>
                                                )}
                    ...
                    {/* H. MORE PERSPECTIVES (PHASE 38: SOCIAL PROOF) */}
                    {(() => {
                        const otherRecommendations = ALL_REGIONAL_ASSETS.filter(a =>
                            a.authorId !== item.authorId &&
                            (a.title === item.title || (a.lat === item.lat && a.lng === item.lng))
                        );
                                ...
                                <div className="space-y-3">
                                    {otherRecommendations.map((rec, idx) => {
                                        ...
                                        return (rec.insiderTip?.teaser || rec.insiderTip?.teaserEn) ? (
                                            <div key={idx} className="flex gap-4 p-5 bg-slate-50 border border-slate-100 rounded-3xl hover:bg-white transition-all hover:shadow-xl hover:shadow-slate-200/50 group">
                                                <div className="relative shrink-0">
                                                    <div className="w-12 h-12 rounded-2xl overflow-hidden shadow-md group-hover:scale-110 transition-transform">
                                                        <img
                                                            src={recAuthor?.avatar || "https://api.dicebear.com/7.x/avataaars/svg?seed=" + rec.authorId}
                                                            alt={recAuthor?.name}
                                                            className="w-full h-full object-cover"
                                                        />
                                                    </div>
                                                ...
                                                <div className="flex flex-col justify-center">
                                                    <div className="text-[10px] font-black text-teal-600 uppercase tracking-widest mb-1">
                                                        {lang === 'en' && recAuthor?.nameEn ? recAuthor.nameEn : (recAuthor?.name || rec.author)}
                                                    </div>
